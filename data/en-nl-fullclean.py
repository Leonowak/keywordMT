...
df = pd.read_parquet("en-nl.parquet")
print(df.head())
df.to_csv("en-nl.csv", index=False)

df = pd.read_csv("en-nl.csv")

df["translation_dict"] = df["translation"].apply(ast.literal_eval)

df["source"] = df["translation_dict"].apply(lambda x: x.get("en", ""))
df["target"] = df["translation_dict"].apply(lambda x: x.get("nl", ""))

df = df.drop(columns=["translation", "translation_dict"])

df.to_csv("en_nl_parallel.csv", index=False)

print(df.head())

df = pd.read_csv("en_nl_parallel.csv")
...
train_df.to_csv("en_nl_train.csv", index=False)
dev_df.to_csv("en_nl_dev.csv", index=False)
test_df.to_csv("en_nl_test.csv", index=False)
