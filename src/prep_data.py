...
def is_valid_candidate(text: str) -> bool:
    if not isinstance(text, str):
        return False
    t = text.strip()
    if len(t) < 2:              # too short to be useful
        return False
    if t in {"news news", "webs", "nltv"}:  # obvious junk examples we saw
        return False
    if "  " in t:               # double spaces
        return False
    return True
...
def main():
    ...
    ap.add_argument("--input_csv",  required=True,
                    help="Enriched candidates (e.g., mbart50_de_en_cands_enriched_clean.csv)")
    ...
    # --- Filter 3: simple junk filter on target ---
    df = df[df["target_norm"].apply(is_valid_candidate)].copy()

    # --- Dedupe by (source, target_norm) to remove repeats across models/runs ---
    df = df.drop_duplicates(subset=["source","target_norm"]).copy()

    # --- Select top-K by actual_volume per source ---
    df = (
    ...
    # --- Keep a "clean_full" for inspection (still has volumes/langs) ---
